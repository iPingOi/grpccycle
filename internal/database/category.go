...
func (c *Category) Find(findId string) (Category, error) {
	var id, name, description string
	err := c.db.QueryRow("SELECT id, name, description FROM categories WHERE id = $1", findId).Scan(&id, &name, &description)
	if err != nil {
		return Category{}, err
	}
	return Category{ID: id, Name: name, Description: description}, nil
